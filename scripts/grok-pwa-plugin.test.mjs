...
/** Isolated cwd so tests never pick up this repo's baked src/lib/og/site.json. */
function emptyWorkspace() {
  const root = mkdtempSync(join(tmpdir(), "grok-pwa-"));
  mkdirSync(join(root, "public"), { recursive: true });
  return root;
}

function bareCtx(overrides = {}) {
  return { cwd: emptyWorkspace(), site: {}, ...overrides };
}
...
test("injects the extensions script without a project id", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", bareCtx({
    appName: "Demo",
    projectId: "",
  }));
...
test("does not duplicate grok:app_id", () => {
  const ctx = bareCtx({ appName: "Demo", projectId: "proj-123" });
...
test("does not duplicate x:creator tags", () => {
  const ctx = bareCtx({ appName: "Demo", projectId: "", creator: "@alice", creatorId: "42" });
  const once = injectGrokPwaHead("<html><head></head></html>", ctx);
  const twice = injectGrokPwaHead(once, ctx);
  assert.equal(once, twice);
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, bareCtx({ appName: "Wild Race" }));
...
test("emits x:game:image for a public host when site.banner is set", () => {
  ...
  const out = injectGrokPwaHead(html, bareCtx({
    host: "wild-race.grok.me",
...
test("does not emit x:game:image without a public host or banner", () => {
  const noHost = injectGrokPwaHead("<html><head></head></html>", bareCtx({
    site: { banner: "/x-banner.jpg" },
  }));
  assert.doesNotMatch(noHost, /x:game:image/);
  const noBanner = injectGrokPwaHead("<html><head></head></html>", bareCtx({
    host: "wild-race.grok.me",
...
test("published VITE_PUBLIC_HOSTNAME wins over request Host for og:image", () => {
  ...
  try {
    const vercelHost = injectGrokPwaHead("<html><head><title>RACK</title></head></html>", bareCtx({
    ...
    const otherPublicHost = injectGrokPwaHead("<html><head><title>RACK</title></head></html>", bareCtx({
...
test("vercel Host without a public hostname emits no og:image", () => {
  ...
  try {
    const out = injectGrokPwaHead("<html><head><title>RACK</title></head></html>", bareCtx({
      host: "01a020b6-803a-71a2-bb47-e2bec57eb9a2-662k8x1l1-xai-org.vercel.app",
      site: { title: "RACK", card: "custom" },
    }));
    assert.doesNotMatch(out, /property="og:image"/);
...
test("emits og:image for a public host and prefers a custom card", () => {
  const placeholder = injectGrokPwaHead("<html><head></head></html>", bareCtx({
    appName: "Wild Race",
    host: "wild-race.grok.me",
    site: { title: "Wild Race" },
  }));
  ...
  const custom = injectGrokPwaHead("<html><head></head></html>", bareCtx({
    appName: "Wild Race",
    host: "wild-race.grok.me",
...
test("placeholder og:image appends site.color when it is 6-digit hex", () => {
  const themed = injectGrokPwaHead("<html><head></head></html>", bareCtx({
    host: "wild-race.grok.me",
  ...
  const invalid = injectGrokPwaHead("<html><head></head></html>", bareCtx({
    host: "wild-race.grok.me",
...
test("injects into documents with no head element", () => {
  const out = injectGrokPwaHead("<html><body>hi</body></html>", bareCtx({ appName: "Solo" }));
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector(bareCtx({ appName: "Wild Race" }));
  const chunks = [
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", bareCtx({ appName: "Wild Race" }));
...
test("streaming injector passes post-head chunks through untouched", () => {
  const injector = createHeadInjector(bareCtx());
